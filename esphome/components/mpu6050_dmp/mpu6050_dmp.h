...
class MPU6050_DMP : public PollingComponent, public i2c::I2CDevice {
 ...
 protected:
  ...
  long quat[4];
  // State vars
  struct gyro_reg_s reg = {.who_am_i = 0x75,
                           .rate_div = 0x19,
                           .lpf = 0x1A,
                           .prod_id = 0x0C,
                           .user_ctrl = 0x6A,
                           ...
                           .prgm_start_h = 0x70};
  struct hw_s hw = {
      .addr = 0x68, .max_fifo = 1024, .num_reg = 118, .temp_sens = 340, .temp_offset = -521, .bank_size = 256};

  struct test_s test = {.gyro_sens = 32768 / 250,
                        .accel_sens = 32768 / 16,
                        .reg_rate_div = 0,     /* 1kHz. */
                        .reg_lpf = 1,          /* 188Hz. */
  ...
  struct gyro_state_s st = {.reg = &reg, .hw = &hw, .test = &test};
  // From inv_mpu.h
