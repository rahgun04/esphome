...
#define MPU6050_DMP_ERROR_CHECK(func) \
  if (!(func)) { \
    this->mark_failed(); \
    return; \
  }
...
void esphome::mpu6050_dmp::MPU6050_DMP::delay_ms(unsigned long ms) {
  delay(ms);
}

void MPU6050_DMP::get_ms(unsigned long *ms) {
  ms[0] = millis();
}
 ...
 *  @return     0 if successful.
 */
int MPU6050_DMP::mpu_read_fifo(short *gyro, short *accel, unsigned long *timestamp,
 ...
 */
int MPU6050_DMP::mpu_read_fifo_stream(unsigned short length, unsigned char *data,
    unsigned char *more)
{
    unsigned char tmp[2];
 ...
 *  @return     0 if successful.
 */
int MPU6050_DMP::mpu_write_mem(unsigned short mem_addr, unsigned short length,
        unsigned char *data)
{
    unsigned char tmp[2];
 ...
 *  @return     0 if successful.
 */
int MPU6050_DMP::mpu_read_mem(unsigned short mem_addr, unsigned short length,
        unsigned char *data)
{
    unsigned char tmp[2];
 ...
 *  @return     0 if successful.
 */
int MPU6050_DMP::mpu_load_firmware(unsigned short length, const unsigned char *firmware,
 ...
 *  @return     0 if successful.
 */
int MPU6050_DMP::dmp_register_tap_cb(void (*func)(unsigned char, unsigned char))
{
    dmp.tap_cb = func;
    return 0;
}
 ...
 *  @return     0 if successful.
 */
int MPU6050_DMP::dmp_read_fifo(short *gyro, short *accel, long *quat,
